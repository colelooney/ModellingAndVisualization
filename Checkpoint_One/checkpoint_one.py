...
class IsingModel:
    ...
    def __init__(self,N,T,dynamic,uncertainty, J,save_fig):
        """
    ...
    def run_data_collection(self):
        ...
        self.T = 3.0
        for i in range(4900): # equilibration steps, warm up time of 5000 sweeps for first grid. 100 extra sweeps in average magnetisation method
            for _ in range(self.N * self.N):
                if self.dynamic == 'glauber':
                    self.glauber_update()
                elif self.dynamic == 'kawasaki':
                    self.kawasaki_update()
        for T in np.arange(3.0, 0.99, -0.1):
            self.T = T
    ...
    def plot_data(self, total_mags, susceptibilities, energies,
        ...
        if self.save_fig:
            plt.savefig(
                f"ising_plots_{self.dynamic}_N{self.N}.png",
                dpi=300,
                bbox_inches="tight"
            )
        plt.show()
    ...
    def plot_stored_data(self):
        ...
        """
        try:
            data = np.loadtxt(f'ising_data_{self.dynamic}_N{self.N}.csv', delimiter=',', skiprows=1)
    ...
    def store_data(self, total_mags, susceptibilities, energies, heat_capacities,heat_capacity_uncertainty, temperatures):
        ...
        np.savetxt(f'ising_data_{self.dynamic}_N{self.N}.csv', data.T, delimiter=',', header='Temperature,Average Magnetisation,Magnetic Susceptibility,Average Energy,Heat Capacity', comments='')
