...
class poisson:
    ...
    def B_field_vector_plot(self):
        ...
        skip = 5
        y_idx, z_idx = np.meshgrid(
            np.arange(0, self.N, skip),
            np.arange(0, self.N, skip), indexing='ij'
        )
        Bx_s = Bx_slice[::skip, ::skip]
        By_s = By_slice[::skip, ::skip]

        mag = np.sqrt(Bx_s**2 + By_s**2)
        mag_safe = np.where(mag == 0, 1, mag)
        Bx_norm = Bx_s / mag_safe
        By_norm = By_s / mag_safe

        log_mag = np.log10(np.where(mag == 0, np.nan, mag))
        norm  = mcolors.Normalize(
            vmin=np.nanpercentile(log_mag, 5),
            vmax=np.nanpercentile(log_mag, 95)
        )
        colors = plt.cm.plasma(norm(log_mag.flatten()))

        fig, ax = plt.subplots(figsize=(8, 8))
        ax.quiver(y_idx, z_idx, Bx_norm, By_norm,
                color=colors, scale=30, scale_units='width',
                width=0.003, headwidth=4, headlength=5, pivot='mid')

        sm = plt.cm.ScalarMappable(cmap=plt.cm.plasma, norm=norm)
        sm.set_array([])
        plt.colorbar(sm, ax=ax, fraction=0.04, pad=0.02,
                    label=r'$\log_{10}|B|$')
        ax.plot(self.N//2, self.N//2, 'b+', markersize=12,
                markeredgewidth=2, label='Wire (z-axis)')
        ax.set_xlabel('x index'); ax.set_ylabel('y index')
        ax.set_title('Magnetic Field Vectors (B) around Wire')
        ax.set_aspect('equal'); ax.legend()
        plt.tight_layout()
        plt.savefig('magnetic_field_vectors.png', dpi=150, bbox_inches='tight')
        plt.show()
    ...
    def vector_plot(self):
        ...
        Ey_s = Ey_slice[::skip, ::skip]
        Ez_s = Ez_slice[::skip, ::skip]

        # Normalise to unit vectors — colour encodes magnitude instead
        mag = np.sqrt(Ey_s**2 + Ez_s**2)
        mag_safe = np.where(mag == 0, 1, mag)
        Ey_norm = Ey_s / mag_safe
        Ez_norm = Ez_s / mag_safe

        # Log-scale colour so near and far field are both visible
        log_mag = np.log10(np.where(mag == 0, np.nan, mag))
        norm  = mcolors.Normalize(
            vmin=np.nanpercentile(log_mag, 5),
            vmax=np.nanpercentile(log_mag, 95)
        ...
        fig, ax = plt.subplots(figsize=(8, 8))
        ax.quiver(y_idx, z_idx, Ey_norm, Ez_norm,
                color=colors, scale=30, scale_units='width',
                width=0.003, headwidth=4, headlength=5, pivot='mid')
        ...
        ax.set_title('Electric Field Vectors (Midplane Slice)')
        ax.set_aspect('equal'); ax.legend()
        plt.tight_layout()
        plt.savefig('electric_field_vectors.png', dpi=150, bbox_inches='tight')
        plt.show()
