...
@njit
def glauber_sweep_numba(grid, N,T,J):
    """
    perform a single sweepf of glauber dynamics using numba for speed up
    """
    for _ in range(N * N):
        i = np.random.randint(0,N)
        j = np.random.randint(0,N)

        neighbour_sum = (grid[(i+1)%N, j] +
                 grid[(i-1)%N, j] +
                 grid[i, (j+1)%N] +
                 grid[i, (j-1)%N])
        delta_energy = 2 * J * grid[i, j] * neighbour_sum

        if delta_energy <= 0:
            grid[i,j] = -grid[i,j]

        elif math.exp(-delta_energy/T) > np.random.rand():
            grid[i,j] = -grid[i,j]

@njit
def kawasaki_sweep_numba(grid, N ,T, J):
    for _ in range(N * N):
        ...
        adjacent = (
            (abs(i1 - i2) == 1 and j1 == j2)
            or (i1 == i2 and abs(j1 - j2) == 1)
            or (i1 == 0 and i2 == N - 1 and j1 == j2)   # periodic wrap rows
            or (i1 == N - 1 and i2 == 0 and j1 == j2)
            or (i1 == i2 and j1 == 0 and j2 == N - 1)   # periodic wrap cols
            or (i1 == i2 and j1 == N - 1 and j2 == 0)
        )
        ...
        if not adjacent:
            # Non-adjacent: neighbour bonds of site 1 and site 2 are independent
            for di, dj in ((-1, 0), (1, 0), (0, -1), (0, 1)):
                ni = (i1 + di) % N
                nj = (j1 + dj) % N
                energy_before += -J * s1 * grid[ni, nj]   # bonds of site 1 before
                energy_after  += -J * s2 * grid[ni, nj]   # bonds of site 1 after swap
 
            for di, dj in ((-1, 0), (1, 0), (0, -1), (0, 1)):
                ni = (i2 + di) % N
                nj = (j2 + dj) % N
                energy_before += -J * s2 * grid[ni, nj]   # bonds of site 2 before
                energy_after  += -J * s1 * grid[ni, nj]   # bonds of site 2 after swap
 
        else:
            # Adjacent: the shared bond must not be double-counted
            for di, dj in ((-1, 0), (1, 0), (0, -1), (0, 1)):
                ni = (i1 + di) % N
                nj = (j1 + dj) % N
                # Use the post-swap occupant of (i2,j2) when that is the neighbour
                neighbour_spin_before = grid[ni, nj]
                neighbour_spin_after  = s2 if (ni == i2 and nj == j2) else grid[ni, nj]
                energy_before += -J * s1 * neighbour_spin_before
                energy_after  += -J * s2 * neighbour_spin_after
 
            for di, dj in ((-1, 0), (1, 0), (0, -1), (0, 1)):
                ni = (i2 + di) % N
                nj = (j2 + dj) % N
                neighbour_spin_before = grid[ni, nj]
                neighbour_spin_after  = s1 if (ni == i1 and nj == j1) else grid[ni, nj]
                energy_before += -J * s2 * neighbour_spin_before
                energy_after  += -J * s1 * neighbour_spin_after
        ...
        if delta_energy <= 0.0:
            grid[i1, j1] = s2
            grid[i2, j2] = s1
        elif np.random.rand() < np.exp(-delta_energy / T):
            grid[i1, j1] = s2
            grid[i2, j2] = s1
...
class IsingModel:
    ...
    def run_data_collection(self):
        ...
        self.update_fn = glauber_sweep_numba if self.dynamic == 'glauber' else kawasaki_sweep_numba
        for i in range(4900): # equilibration steps, warm up time of 5000 sweeps for first grid. 100 extra sweeps in each temp
            # for _ in range(self.N * self.N):
            self.update_fn(self.grid,self.N,self.T,self.J)
